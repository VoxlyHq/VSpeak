...
#        n_embd = 8192
    ...
    @staticmethod
    def load(model_plus: ModelPlus, prefix: str) -> Params:
        ...
        if hf_config_path.exists():
            ...
            params = Params.guessed(model_plus.model, prefix)
        else:
...
def merge_multifile_models(models_plus: list[ModelPlus], prefix) -> ModelPlus:
...
class LazyUnpickler(pickle.Unpickler):
    ...
    CLASSES: dict[tuple[str, str], Any] = {
        ...
        ('torch', 'LongStorage'): LazyStorageKind(DT_F32), #TODO wtf is this, maybe an int64?
...
def convert_model_names(model: LazyModel, params: Params, prefix: str, outprefix: str) -> LazyModel:
    ...
    for name, lazy_tensor in model.items():
        ...
        if name_new is None:
            print(f"Unexpected tensor name: {name}")
            continue
...
def load_some_model(path: Path, prefix:str) -> ModelPlus:
    ...
    model_plus = merge_multifile_models(models_plus, prefix)
    return model_plus
